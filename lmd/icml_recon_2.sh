...
python recon.py --config configs/adj/icmlface_configs_2_id.py\
  --ckpt_path results/icmlface_adj_2/checkpoints/checkpoint_60.pth --in_domain ADJ \
  --out_of_domain ADJ --batch_size 50 --mask_type checkerboard_alt --mask_num_blocks 8 \
  --reps_per_image 10 --workdir results/icmlface_adj_2/ID_vs_OOD/
